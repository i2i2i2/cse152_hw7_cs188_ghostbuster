    ...
    def elapseTime(self, gameState):
        ...
        *** GORY DETAIL AHEAD ***
        ...
        for p, prob in self.beliefs.items():
            ...
            for p1, prob1 in newPosDist.items():
                allPossible[p1] += prob1 * prob
        allPossible.normalize()
        self.beliefs = allPossible
...
class ParticleFilter(InferenceModule):
    ...
    def initializeUniformly(self, gameState):
        ...
        while len(self.particles) < self.numParticles:
            particleLeft = self.numParticles - len(self.particles)
            if particleLeft > len(self.legalPositions):
                self.particles += self.legalPositions
            else:
                self.particles += self.legalPositions[0:particleLeft]
    ...
    def observe(self, observation, gameState):
        ...
        for pt in self.legalPositions:
            dist = util.manhattanDistance(pt, pacmanPosition)
            allPossible[pt] = emissionModel[dist] * beliefDist[pt]

        allZero = True
        for _, prob in allPossible.items():
            if prob != 0:
                allZero = False
                break
    ...
    def elapseTime(self, gameState):
        ...
        for i, pt in enumerate(self.particles):
            posDist = self.getPositionDistribution(self.setGhostPosition(gameState, pt))
            self.particles[i] = util.sample(posDist)
...
class JointParticleFilter:
    ...
    def observeState(self, gameState):
        ...
        "*** YOUR CODE HERE ***"
        allPossible = util.Counter()
        # calc probability for each set
        for ptSet in self.particles:
            ptList = list(ptSet)
            prob = 1
            for i in range(self.numGhosts):
                if noisyDistances[i] == None:
                    ptList[i] = self.getJailPosition(i)
                else:
                    prob *= emissionModels[i][util.manhattanDistance(pacmanPosition, ptList[i])]
            allPossible[tuple(ptList)] += prob

        # check if all probability is 0
        found = False
        for _, prob in allPossible.items():
            if prob != 0:
                found = True
    ...
    def elapseTime(self, gameState):
        ...
        for oldParticle in self.particles:
            ...
            for i, pt in enumerate(newParticle):
                posDist = getPositionDistributionForGhost(gameState, i, self.ghostAgents[i])
                newParticle[i] = util.sample(posDist)
